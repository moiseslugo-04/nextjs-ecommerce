...
import { deleteVerificationTokenByUserId } from '@lib/features/verificationToken/verification-token-repository'
import { generateVerificationToken } from '@features/verificationToken/verification-token.service'
export type VerificationTokenTypes = 'email_verification' | 'reset_password'
...
export async function resendVerificationToken(
  ...
  await deleteVerificationTokenByUserId(user.id, type)
  const token = await generateVerificationToken({
    userId: user.id,
    type,
    identifier: user.email,
